...
public class RegionWiseAgeAnalysisExcelReport {
	...
	public RegionWiseAgeAnalysisExcelReport(OmsAlertMasterDto alertDto) {
	...
	private void writeCommonHeaderLine(UserDto userDto) {
		try {
			...
			DateFormat outputFormatter2 = new SimpleDateFormat("dd/MM/yyyy");
			String todaysDateTemp = outputFormatter2.format(new Date());
			cell6.setCellValue(todaysDateTemp);
	...
	private void writeHeaderLine() {
		...
		createCell(row, 1, "Zone", style);
		createCell(row, 2, "Region", style);
		createCell(row, 3, "1 Day", style);
		createCell(row, 4, ">1 Day & <= 1 Week", style);
		createCell(row, 5, ">1 Week & <= 2 Week", style);
	...
	private void writeDataLines() {
		...
		// style.setWrapText(true);
		int srNo = 1;
		for (OmsAlertMasterDto alertObj : alertDto.getAlertList()) {
			XSSFRow row = sheet.createRow(rowCount++);
			int columnCount = 0;

			createCell(row, columnCount++, srNo, style);
			createCell(row, columnCount++, alertObj.getZone(), style);
			createCell(row, columnCount++, alertObj.getRegion(), style);
			...
			createCell(row, columnCount++, alertObj.getPerRuleCountTotal(), style);
		...
		createCell(rowT, columnCountTemp++, alertDto.getGrandTotal(), styleName);
