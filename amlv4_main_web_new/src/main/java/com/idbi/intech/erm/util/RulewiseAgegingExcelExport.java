...
public class RulewiseAgegingExcelExport {
	...
	public RulewiseAgegingExcelExport(OmsAlertMasterDto alertDto) {
	...
	private void writeCommonHeaderLine(UserDto userDto) {
		try {
			...
			DateFormat outputFormatter12 = new SimpleDateFormat("dd/MM/yyyy");
			String todaysDate1 = outputFormatter12.format(new Date());
			cell6.setCellValue(todaysDate1);
	...
	private void writeHeaderLine() {
		...
		createCell(row, 1, "Rule Id", style);
		createCell(row, 2, "Rule Description", style);
		createCell(row, 3, "Classification", style);
		createCell(row, 4, "Risk Severity", style);
		createCell(row, 5, "1 Day", style);
		createCell(row, 6, ">1 Day & <= 1 Week", style);
		createCell(row, 7, ">1 Week & <= 2 Week", style);
		createCell(row, 8, ">2 Week & <= 1 Month", style);
		createCell(row, 9, ">1 Month & <= 3 Month", style);
		createCell(row, 10, ">3 Month & <= 6 Month", style);
		createCell(row, 11, ">6 Month & <= 1 Year", style);
		createCell(row, 12, "> 1 Year", style);
		createCell(row, 13, "Grand Total", style);
	...
	private void writeDataLines() {
		...
		// style.setWrapText(true);
		int srNo = 1;
		for (RuleMasterDto alertObj : alertDto.getRuleList()) {
			XSSFRow row = sheet.createRow(rowCount++);
			int columnCount = 0;

			createCell(row, columnCount++, srNo, style);
			createCell(row, columnCount++, alertObj.getRuleId(), style);
			createCell(row, columnCount++, alertObj.getRuleDesc(), styleName);
			createCell(row, columnCount++, alertObj.getRuleClassification(), style);
			createCell(row, columnCount++, alertObj.getRulePriority(), style);
			createCell(row, columnCount++, alertObj.getOneDayCount(), style);
			createCell(row, columnCount++, alertObj.getOneWeekCount(), style);
			createCell(row, columnCount++, alertObj.getGtOneWeekltTwoWeek(), style);
			createCell(row, columnCount++, alertObj.getGtTwoWeekltOneMonth(), style);
			createCell(row, columnCount++, alertObj.getGtOneMonthltThreeMonth(), style);
			createCell(row, columnCount++, alertObj.getGtThreeMonthltSixMonth(), style);
			createCell(row, columnCount++, alertObj.getGtSixMonthltOneYear(), style);
			createCell(row, columnCount++, alertObj.getGtOneYears(), style);
			createCell(row, columnCount++, alertObj.getPerRuleCount(), style);
		...
		XSSFRow rowT = sheet.createRow(rowCountTemp++);
		createCell(rowT, columnCountTemp++, srnoTemp++, style);
		createCell(rowT, columnCountTemp++, "", style);
		createCell(rowT, columnCountTemp++, "", style);
		createCell(rowT, columnCountTemp++, "", style);
		createCell(rowT, columnCountTemp++, "TOTAL", styleName);
		createCell(rowT, columnCountTemp++, alertDto.getOneDayCountTotal(), styleName);
		createCell(rowT, columnCountTemp++, alertDto.getOneWeekCountTotal(), styleName);
		createCell(rowT, columnCountTemp++, alertDto.getGtOneWeekltTwoWeekTotal(), styleName);
		createCell(rowT, columnCountTemp++, alertDto.getGtTwoWeekltOneMonthTotal(), styleName);
		createCell(rowT, columnCountTemp++, alertDto.getGtOneMonthltThreeMonthTotal(), styleName);
		createCell(rowT, columnCountTemp++, alertDto.getGtThreeMonthltSixMonthTotal(), styleName);
		createCell(rowT, columnCountTemp++, alertDto.getGtSixMonthltOneYearTotal(), styleName);
		createCell(rowT, columnCountTemp++, alertDto.getGtOneYearsTotal(), styleName);
		createCell(rowT, columnCountTemp++, alertDto.getPerRuleCountTotal(), styleName);
