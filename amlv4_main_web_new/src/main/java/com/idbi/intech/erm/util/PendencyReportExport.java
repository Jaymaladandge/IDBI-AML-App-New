...
public class PendencyReportExport {
	...
	public PendencyReportExport(NotificationDto notificationDto)
	...
	private void writeHeaderLine2() {
		...
		createCell(row, 4, "Pending KRI", style);
	...
	private void writeHeaderLine3() {
		Row row = sheet.createRow(rowCount2+1);
		int rownum=rowCount2+1;
		...
		createCell(row, 4, "Pending for Verification KRI", style);

		createCell(row, 6, "Assessment Value", style);
	...
	private void writeDataLines3() {
		...
		List<NotificationDto> ntfPendingForVerificationListDto=new ArrayList<>();
		  notificationDto.getNtfDtoListPendingForVerification().stream().sorted((data1,data2)->data1.getDeptName().compareTo(data2.getDeptName())).forEach(ntfPendingForVerificationListDto::add);

		for (NotificationDto notificationDto : notificationDto.getNtfDtoListPendingForVerification()) {
	...
	private void writeHeaderLine4() {
		Row row = sheet.createRow(rowCount3+1);
		int rownum=rowCount3+1;
		...
		sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 4, 5));
		createCell(row, 4, "Captured KRI", style);
	...
	private void writeDataLines4() {
		...
		style2.setBorderTop(BorderStyle.THIN);
		style2.setTopBorderColor(IndexedColors.BLACK.getIndex());
		List<NotificationDto> ntfCapturedListDto=new ArrayList<>();
		  notificationDto.getNtfDtoListCaptured().stream().sorted((data1,data2)->data1.getDeptName().compareTo(data2.getDeptName())).forEach(ntfCapturedListDto::add);

		for (NotificationDto notificationDto : ntfCapturedListDto) {
	...
	private void writeDataLines2() {
		...
		style2.setBorderTop(BorderStyle.THIN);
		style2.setTopBorderColor(IndexedColors.BLACK.getIndex());
		List<NotificationDto> ntfPendingListDto=new ArrayList<>();
		  notificationDto.getNtfDtoListPending().stream().sorted((data1,data2)->data1.getDeptName().compareTo(data2.getDeptName())).forEach(ntfPendingListDto::add);

		for (NotificationDto notificationDto : notificationDto.getNtfDtoListPending()) {
