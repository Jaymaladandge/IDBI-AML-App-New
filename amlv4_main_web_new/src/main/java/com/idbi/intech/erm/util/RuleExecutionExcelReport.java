...
public class RuleExecutionExcelReport {
	...
	public RuleExecutionExcelReport(OmsAlertMasterDto alertDto) {
	...
	private void writeCommonHeaderLine(UserDto userDto) {
		try {
			...
			cell2.setCellValue("RULE WISE EXECUTION COUNT REPORT");
	...
	private void writeHeaderLine() {
		...
		createCell(row, 3, "Rule Category", style);
		createCell(row, 4, "Risk Severity", style);
		createCell(row, 5, "Rule Frequency", style);
		createCell(row, 6, "Fisrt Date", style);
		createCell(row, 7, "Second Date", style);
		createCell(row, 8, "Third Date", style);
	...
	private void writeDataLines() {
		...
		// style.setWrapText(true);
		int srNo = 1;
		for (OmsAlertMasterDto alertObj : alertDto.getRuleAndDayWiseCount()) {
			...
			createCell(row, columnCount++, alertObj.getRuleFrequency(), style);
			createCell(row, columnCount++, alertObj.getFirstCount(), style);
			createCell(row, columnCount++, alertObj.getSecondCount(), style);
			createCell(row, columnCount++, alertObj.getThirdCount(), style);
